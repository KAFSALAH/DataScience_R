...
for Immigrants to Canada From 1980 to 2013 Per Decade?")),
                        ...
                        ,
                        tabPanel("FUTURE PREDICTION",column(12,
                                             h3("Q6 - Which Countries in The Future Will Have More Immigrants,
and Which Will Have Less?")),
                                 selectInput("country3","Please Select a Country",
                                             na.omit(unique(df$Country))),
                                 plotOutput("Q6")
                        )
                        ,
                        tabPanel("DOMINANCE",column(12,
                                             h3("Q7 - Which Countries Would Have a Dominant Immigrants (Population)?")),
                                             h4("In the word cloud, the bigger the word the more numbers are traveled from that country. The waffle chart on the left shows the top 8 countries relative values. On the right, the user can select between 3 countries to make relative comparisons."),
                                 
                                 plotOutput("Q7_1"),
                                 column(textInput("a1", "Country No.1:", ""),width = 4),
                                 column(textInput("a2", "Country No.2:", ""),width = 4),
                                 column(textInput("a3", "Country No.3:", ""),width = 4),
                                 plotOutput("Q7_2"),
                                 plotOutput("Q7_3")
                        )
                        ,
                        tabPanel("CONTINENTS TRENDS",column(12,
             ...
             tabPanel("SUMMARY",
                      fluidPage(
                        ...
                        fluidRow(
                          ...
                          column(12,
                                 h3("Outlines - Group C"),
                                 h4("This project encompasses descriptive and exploratory analysis on immigration counts to Canada from 1980 to 2013. The dataset is presented by The United Nations - Population Division - Department of Economic and Social Affairs.")
                                 ), 
                          column(2),
                        ),
                        #reflection
                        fluidRow(
                          column(12,
                                 h4("The user can select between 8 tabs wherein analytical visuals are presented. Each tab represents an analytical question regarding the immigration trends to Canada."),
                                 h4("Our Reflection: In the process of answering these questions, which is accomplished through the search in the shiny development document, the trend of the world countries is queried according to the requirements of the question."))
                        )
                      )
             )
  )
